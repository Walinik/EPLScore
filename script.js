...
function renderChatList() {
    ...
    // Затем личные чаты с сотрудниками
...
function renderAdminTable() {
    ...
    employees.forEach(emp => {
        ...
        lvlSel.id = `lvl_${id}`;
        for (let i=1;i<=5;i++) { const opt = document.createElement('option'); opt.value=i; opt.textContent=i; if(emp.level==i) opt.selected=true; lvlSel.appendChild(opt); }
        lvlCell.appendChild(lvlSel);
        const adminCell = row.insertCell(4);
        const chk = document.createElement('input'); chk.type='checkbox'; chk.checked=emp.is_admin===true; chk.id=`admin_${id}`;
        adminCell.appendChild(chk);
        const actCell = row.insertCell(5);
        const saveBtn = document.createElement('button'); saveBtn.textContent='💾 Сохранить'; saveBtn.className='btn-sm';
        saveBtn.onclick = async () => {
            ...
            if (empTarget?.full_name === 'Системный Администратор' && !is_admin) { showToast('❌ Нельзя снять права', true); return; }
            const response = await supabasePatch('employees', id, { full_name, position, password, level: parseInt(level), is_admin });
            if (response.ok) { showToast(`✅ Обновлено`); await loadEmployees(); renderAdminTable(); renderChatList(); }
            else showToast('❌ Ошибка', true);
        };
        const delBtn = document.createElement('button'); delBtn.textContent='🗑️ Удалить'; delBtn.className='btn-sm btn-del';
        delBtn.onclick = async () => {
            const empTarget = employees.find(e => e.id === id);
            if (!empTarget) return;
            if (empTarget.full_name === 'Системный Администратор') { showToast('❌ Нельзя удалить', true); return; }
...
async function registerEmployee() {
    ...
    if (!full || !pos || !pwd) { showToast('❌ Заполните поля', true); return; }
    if (employees.find(e => e.full_name === full)) { showToast('❌ Сотрудник уже есть', true); return; }
    const newEmp = { id: Date.now().toString(), full_name: full, position: pos, password: pwd, level: parseInt(lvl), is_admin: false, display_name: full, epls_name: '🤖 EPLS' };
...
async function login() {
    ...
    if (!fullname || !password) { showToast('Введите ФИО и пароль', true); return; }
    const btn = document.getElementById('loginBtn');
    btn.disabled = true; btn.textContent = 'ВХОД...';
    await loadEmployees();
    const user = employees.find(e => e.full_name === fullname && e.password === password);
    if (!user) { showToast('❌ Неверные данные', true); btn.disabled = false; btn.textContent = 'ВОЙТИ'; return; }
    currentUser = user;
    ...
    if (isAdmin) {
        ...
        const admin = employees.find(e => e.is_admin === true);
        if (admin) currentRoom = getPrivateRoom(currentUser, admin);
        else { showToast('❌ Администратор не найден', true); btn.disabled = false; btn.textContent = 'ВОЙТИ'; return; }
    }
    lastMessageTimestamp = 0;
    renderChatList();
    await loadFullMessages();
    ...
    refreshInterval = setInterval(async () => {
        if (currentUser) {
            ...
            if (currentUser.is_admin) {
                ...
                await loadEmployees();
                if (oldCount !== employees.length) { renderAdminTable(); renderChatList(); }
            }
        }
    }, 3000);
